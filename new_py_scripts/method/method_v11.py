...
# results = client.query_archive(query={
#   'upload_id': ['-3PKzkxDQ6GLKycLSDzB7A'],
#     'calc_id': ['-PhHhxhPd55lSP_vkQycn7s0IQqE']}, 
#         required={
#         'section_run': '*',
    ...
    if i not in un:
        ...
        if i <length:
            ...
            if esm != None:
                ...
                for j in range(len(esm)):
                    # print(j)
                    u = esm[j]
                    
                    for attesm, valueesm in u.__dict__.items():
                        if attesm.startswith(('electronic_structure_method')):
                            # print(att)
                            c = 0
                            meta = []
                            properties = []
                ...
                # defining list for storing attributes  
                    ...
                    for att, value in u.__dict__.items():
                        if not att.startswith(('m','_','electronic_structure_method')):
                            # print('check_here')
                            # print(att)
                            c = 0
                            meta = []
                            ...
                            try:
                                ...
                                section = 'section_method' + '_' + str(j)
                                
                                g.add((URIRef(matvoc+m), RDF.type, URIRef(sosa + 'Sampling')))
                                g.add((URIRef(matvoc+m), URIRef(sosa + 'madeBySampler'), URIRef(nomad + dft_class)))
                                # g.add((URIRef(nomad + dft_class), RDFS.label, Literal(attesm)))
                                s = BNode()
                                y = BNode()
                                g.add((URIRef(matvoc+m), URIRef(nomad + section), s))
                            ...
                            try:
                                ...
                                if properties[c] not in l1 and  properties[c] not in l2:
                                    ...
                                    # a = 'mat' +str(i+1)
                                    a = 'mat'+ '_' + formula_name + '_' + pid_name
                                    section = 'section_method' + '_' + '_' + str(j)
                                    s = BNode()
                                                        ...
                                                        try:
                                                            ...
                                                            for att3,val3 in  value.__dict__.items():
                                                                ...
                                                                if not att3.startswith('m'):
                                                                    ...
                                                                    y2 = BNode()
                                                                    g.add((URIRef(matvoc+m), URIRef(nomad  + section), s))
                                                                    g.add((s, URIRef(nomad+properties2[c]), y2))
                                                    ...
                                                    for x1 in x:
                                                        for att2, val2 in x1.__dict__.items():
                                                            if not att2.startswith(('m','_')):
                                                                ...
                                                                g.add((URIRef(nomad + dft_class), RDF.type, URIRef(sosa + 'Sampler')))
                                                                g.add((URIRef(nomad + dft_class), RDFS.label, Literal('electronic_structure_method',lang ='en')))
